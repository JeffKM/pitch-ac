...
## Phase NW: NEWS 이적뉴스 큐레이션 — 예정 (우선순위 7)
...
> **데이터 소스**: 에펨코리아 해외축구 게시판 정보/SNS 카테고리 (cheerio HTML 스크래핑, 비로그인 접근 확인됨)
...
| **크롤링 도구**    | cheerio (Playwright 불필요, 비로그인 접근 확인됨)                           |
...
- `rate-limiter.ts` — 요청 간 최소 3초 간격 (봇 감지 회피)
- `client.ts` — `fmkoreaFetch(path)`: 브라우저 User-Agent, timeout, rate limiter 연동
  - 패턴 참조: `lib/api/football-data/client.ts`
- `types.ts` — `FmkRawPost`, `FmkRawArticle` (Raw 크롤링 타입)
- `parsers.ts` — cheerio HTML 파싱
  - `parsePostList(html)`: 리스트 페이지 파싱 → 제목/메타 추출
  - `parseArticleBody(html)`: 본문에서 모든 외부 링크 추출
- `classifiers.ts` — 소스 유형 판별
  - `classifySourceType(urls)`: 추출된 URL들의 도메인을 화이트리스트와 매칭 → `SourceType` 반환
  - 우선순위: tweet > article > video > summary
- `mappers.ts` — `mapToTransferNewsItem(post, article?)` Raw → App 타입 변환
- `filters.ts` — 금칙어 필터링
  - `isBannedPost(title)`: 제목에 금칙어 포함 여부 검사
- `index.ts` — re-export

### Task NW102: 크롤링 스크립트 (`scripts/sync-news/`)

- `scripts/sync-news/main.ts` — 메인 실행 스크립트
  - 패턴 참조: `scripts/scraper/main.ts` (ScoutLab 스크래퍼)
- 실행 흐름:
  1. 정보/SNS 리스트 페이지 크롤링 (1회 요청)
  2. 각 글 제목 → 금칙어 필터링
  3. DB 조회 → 이미 있는 글은 메타데이터만 upsert
  4. 새 글만 → 본문 크롤링 (3초 간격) → 링크 추출 → source_type 태깅
  5. 소스 링크 없는 글은 스킵 (자동 수집 대상 아님)
  6. DB upsert + `sync_logs` 기록
...
- `lib/hooks/use-transfer-news.ts`
  ...
  - 패턴 참조: `lib/hooks/use-matchday-fixtures.ts`
...
- `app/(app)/news/_components/`
  ...
  - `source-link-badge.tsx` — 소스 타입별 아이콘 + 텍스트 뱃지
...
**최종 업데이트**: 2026-05-26 (Phase S6 — 포지션 불일치 수정 + 팀 필터 리그 연동)
**진행 상황**: Phase 1~5A 레거시 완료 ✅ | Phase 6~7E 완료 ✅ | Phase S1~S5 완료 ✅ | Phase N1~N2 완료 ✅ | Phase AF+FD 완료 ✅ | Phase HP+HP2+HP3+HP4 완료 ✅ | Phase RK+RK2+PD 완료 ✅ | Phase UX1~UX2 완료 ✅ | Phase MD 완료 ✅ | Phase SB 완료 ✅ | **Phase S6 진행 중** → RK3 → MP → NW → S7 → S8 → CV
