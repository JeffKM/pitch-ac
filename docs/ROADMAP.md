...
- **ScoutLab**: 60+ 고급 메트릭 기반 스카우팅 분석 (10개 뷰, 5대 리그 25/26 1,843명 + 24/25 PL 아카이브 364명, 총 2,207명)
...
- ScoutLab: Action Maps+메트릭 5대 리그 1,843명 전원 완료(S8′01 ✅ 08-01, S8′06 ✅ 08-01, 메트릭 갭 363명 → 0명). 24/25 시즌 PL 아카이브 364명 추가 완료(S8′02 ✅ 08-02, OCR 갭 0건). Similarity는 리그별로 일부 누락(예: Serie A 387/399) — 원인 확인 필요, 백로그 등록
...
### S8′02 이관 — OCR·데이터 품질 (우선순위 낮음)

- 25/26 시즌 crosses zero 85건 소스 데이터 정확성 교차검증: 24/25·25/26 모두 동일 패턴(crosses 84건+carries 1건)의 zero 85건이 존재 — 이는 "소스가 시도 0인 카테고리를 카드 헤더에서 생략 → 파이프라인이 0으로 정확히 기록"하는 정상 동작으로 원인 규명됨(파이프라인 결함 아님). 다만 크로스를 자주 시도하는 FB·AM/W 포지션에서도 0이 나오는 사례가 섞여 있어 **소스 데이터 자체의 정확성**은 별도 확인 필요. 25/26은 시즌 초반 표본 부족 영향도 있을 수 있어 시즌 진행 후 재확인
- OCR 후처리 실패 집계 단위 개선: `extract-action-text.ts`의 성공/실패 카운터가 "선수 단위"라 행 단위 부분 추출 누락(예: crosses만 미매칭)이 "실패 0건"으로 보고됨. S8′02에서는 결과적으로 갭이 아니었던 것으로 확인됐으나(Kristiansen 크로스 0건은 소스 자체 생략), 집계 단위 문제 자체는 실재하며 다음엔 진짜 갭을 놓칠 수 있음 — 행 단위 집계로 개선 필요
...
- **Task S8′02: 24/25 시즌 PL 전체 스크래핑 (구 S801)** ✅ (2026-08-02)
  - 결과: PL 20팀 전원 수집, distinct 선수 364명. Action Maps 1,092건 + 메트릭 1,456건(364명 × per90/total × padj/raw 4조합) 수집, Action Maps↔메트릭 상호 누락 0명·빈 jsonb 0건
  - OCR 후처리(`npm run extract:action-text`) 364명 전원 성공(실패 0건, 소요 565.7초)으로 1,092건 중 1,007건 해소. 잔여 85건 전량 재검증 결과 **전부 실제 0값으로 확정**(crosses 84+carries 1, 주로 CB 포지션 — 크로스 시도 없음이 실제 값). Victor Kristiansen(Leicester, FB) 건도 재시도해 원인 규명: OCR 정규식 미매칭이 아니라 원본 이미지 자체에 Crosses 패널이 없음(OCR 텍스트 `"Carries (21 | 0.8 P90) Passes (60 | 2.2 P90)"`, 원본 이미지 육안 확인으로 3패널 레이아웃 중 Carries·Passes 2개만 렌더링되고 Crosses 자리는 빈 여백 — 시도 0인 카테고리를 소스가 이미지 생성 단계에서 통째로 그리지 않음, Sharp 이진화·리사이즈 등 전처리 손실 가능성은 배제). DB의 0/0 값이 정확하며 추가 조치 불필요, 재처리 시 carries/passes 불변·crosses 행 `updated_at` 미갱신·타 레코드 갱신 0건으로 검증 완료. **OCR 미해결 갭 0건**
  - **참고(스크래퍼 특성, 원본 이미지 육안 확인으로 검증됨)**: ScoutLab 소스는 시도 횟수 0인 액션 카테고리는 헤더 텍스트뿐 아니라 해당 액션의 피치 패널 전체를 그리지 않는다 — 향후 다른 리그·시즌 수집 시 이미지의 패널 개수로도 zero 레코드를 판별할 수 있으며, 이를 OCR 실패로 오진하지 않도록 유의
  - comparison_position 분포 정상(AM/W 384·CB 304·FB 284·FW 152·MF 332, NULL·오타 없음), 값 sanity(음수·이상값·NULL 0건, 25/26과 자릿수 일치) 확인
  - **25/26 무손상 검증**: 전체 1,116건/zero 85건/distinct 372명이 실행 전후 완전 일치, 실행 시간대에 갱신된 25/26 레코드 0건·다른 리그·시즌 갱신 0건
  - 부수 성과(커밋 완료): `abc1132` 🐛 ScoutLab 스크래퍼 시즌·선수 카드 검증 강화(카드 앵커링 `resolveCardScope`+폴링 재검증 `waitForStableCard`로 Streamlit 재렌더 과도기 오탐 차단, `upsertPlayer`가 CLI `--season` 불일치 시 throw, 이미지 업로드 실패 시 조용한 폴백 대신 throw, `SeasonMismatchError`/`PlayerMismatchError`/`EmptyMetricsError` 도입) / `ab6622f` ✨ 배치·fixup 러너(`run-s8-02-batch.sh`, `run-s8-02-fixup.sh`) 추가
  - 후속 발견 2건은 백로그 "S8′02 이관" 섹션에 등록
...
**최종 업데이트**: 2026-08-02 (S8′02 완료 — 24/25 시즌 PL 364명 Action Maps+메트릭 수집, ScoutLab 총 2,207명)
