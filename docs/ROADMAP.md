...
- **Task 025: React cache()를 활용한 요청 중복 제거**
  - `generateMetadata`와 페이지 컴포넌트가 동일 데이터를 요청하는 4곳 개선:
    - `matchday/page.tsx` — `getCurrentGameweek()` 이중 호출 (DB 쿼리 6개 → 3개)
    - `players/[playerId]/page.tsx` — `getPlayerById()` + `getTeamsByIds()` 이중 호출
    - `matchday/[fixtureId]/page.tsx` — `getFixtureById()` + 관련 데이터 이중 호출
  - `import { cache } from "react"` 기반 요청 범위 내 캐싱 적용
  - **의존성**: 없음

- **Task 026: 경기 상세 데이터 로딩 중복 제거**
  - `app/(app)/matchday/[fixtureId]/page.tsx`와 `app/api/matchday/fixture/route.ts`에 동일한 fixture 조립 로직 중복
  - `lib/services/fixture-detail-service.ts`로 공통 함수 추출
  - **의존성**: 없음

- **Task 027: 레포지토리 레이어 server-only 강화**
  - `lib/repositories/*.ts` 전체에 `import "server-only"` 추가
  - `lib/services/sync/*.ts`에도 동일 적용
  - 클라이언트 번들 포함 방지 빌드 타임 검증
  - **의존성**: 없음

- **Task 028: CSP(Content-Security-Policy) 헤더 추가**
  - `next.config.ts`에 CSP 헤더 설정 (report-only 모드로 시작)
  - `img-src`: `cdn.sportmonks.com` 허용
  - `connect-src`: `*.supabase.co` 허용
  - 추후 strict CSP(nonce 기반)로 전환
  - **의존성**: 없음

- **Task 029: E2E 테스트 자동화 (Playwright 코드)**
  - 현재 Playwright MCP 직접 검증 → 자동화된 Playwright 테스트 코드로 전환
  - 10개 핵심 시나리오 커버 (매치데이 리디렉션, GW 네비, 경기 상세, 선수 검색, 비교, 테마, 모바일, 404)
  - CI/CD 파이프라인 연동 (GitHub Actions)
  - **의존성**: 없음

- **Task 030: 단위 테스트 (Vitest)**
  - `lib/repositories/mappers.ts` — 타입 변환 함수 테스트
  - `lib/services/sync/calculate-context.ts` — 순위/백분위 계산 테스트
  - `lib/api/sportmonks/mappers.ts` — API 매퍼 테스트
  - `app/(app)/matchday/_utils.ts` — 날짜 유틸 테스트
  - **의존성**: 없음

- **Task 031: 에러 모니터링 연동 (Sentry)**
  - `@sentry/nextjs` 설정
  - `error.tsx:17`의 `console.error` → Sentry 리포팅 연결
  - 서버/클라이언트 에러 분리 캡처
  - 성능 모니터링 (Web Vitals)
  - **의존성**: 없음
...
**📅 최종 업데이트**: 2026-03-21
**📊 진행 상황**: Phase 1 완료 ✅ | Phase 2 완료 ✅ | Phase 3 완료 ✅ | Phase 4 완료 ✅ | Phase 5-A 진행 중

- **완료**: Task 001~012 (12개), Task 015~024 (10개), Phase 3 코드 리뷰 후속 수정, Task 013·014 생략 결정
- **Phase 5-A (안정성/품질)**: Task 024 완료 ✅, Task 025~031 예정
