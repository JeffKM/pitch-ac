...
## Phase S6: ScoutLab 5대 리그 확장 — 예정

"5-League Data Hub"를 표방하면서 PL만 있는 현재 상태 해소. 4개 리그 전체 스크래핑 + PL Similarity 전체 수집.

- **Task S601: La Liga 전체 스크래핑**
  - `npm run scrape:scoutlab -- --league="La Liga" --skip-positions`
  - per90/padj 기본 조합 우선, 이후 3개 조합 추가
  - 예상: ~350명, ~4시간

- **Task S602: Serie A 전체 스크래핑**
  - `npm run scrape:scoutlab -- --league="Serie A" --skip-positions`
  - 예상: ~350명, ~4시간

- **Task S603: Bundesliga 전체 스크래핑**
  - `npm run scrape:scoutlab -- --league="Bundesliga" --skip-positions`
  - 예상: ~300명 (18팀), ~3.5시간

- **Task S604: Ligue 1 전체 스크래핑**
  - `npm run scrape:scoutlab -- --league="Ligue 1" --skip-positions`
  - 예상: ~300명 (18팀), ~3.5시간

- **Task S605: PL Similarity 전체 수집**
  - `npm run scrape:scoutlab -- --similarity-only --league="Premier League"`
  - 현재 3명만 수집 → 359명 전원
  - 예상: ~6시간

- **Task S606: 5대 리그 Similarity 수집**
  - 4개 리그 similarity-only 순차 실행
  - 예상: ~20시간
...
## Phase UX1: ScoutLab 한국어 맥락 부연 + 팝오버 — 예정 (우선순위 1)
...
- **Task UX101: Glossary 데이터 한국어 리라이트**
  - `scoutlab-glossary-data.ts`의 52개 메트릭 설명을 자연스러운 맥락적 한국어로 전면 재작성
  - 원본 ScoutLab Glossary 탭 내용 참고하되 번역기 투 X
  - 예: "Expected Goals. 슈팅 위치·각도·상황 등을 고려한 기대 득점 수." → "이 위치에서 슈팅하면 보통 골이 될 확률이 얼마나 될까? xG가 높을수록 좋은 슈팅 기회를 많이 만들고 있다는 뜻"

- **Task UX102: 메트릭 한국어 부연 표시 시스템**
  - `format-metric.ts` 확장: 메트릭 키 → 한국어 짧은 부연 매핑 (예: "xG Per Shot" → "슈팅당 기대골")
  - `metric-category-table.tsx`: 메트릭명 옆에 괄호로 한국어 부연 표시
  - 모든 ScoutLab 탭(10개)에서 메트릭명이 표시되는 곳에 일괄 적용

- **Task UX103: 팝오버 상세 해설 컴포넌트**
  - `metric-popover.tsx` 신규: 메트릭명 클릭/호버 시 상세 해설 팝오버
  - Glossary 데이터에서 해당 메트릭 설명 자동 연동
  - CLAUDE.md 규칙 "축구 전문 용어(xG, xA 등)에는 설명 팝오버 제공" 이행

---

## Phase UX2: ScoutLab 코믹 차트 디자인 — 예정 (우선순위 2)
...
## Phase MD: MATCHDAY 자동 동기화 — 예정 (우선순위 3)

> **목표**: 경기 결과가 수동 동기화 없이 자동 반영되도록 킥오프+2.5h 오프셋 기반 Cron 구현.
> **참고**: docs/adr/0002-matchday-auto-sync.md

- **Task MD101: 동적 Cron 스케줄 계산 로직**
  - fixtures 테이블에서 오늘 킥오프 시간 조회
  - 킥오프 + 2.5시간 시점에 동기화 트리거할 타이밍 목록 생성
  - 경기 없는 날은 스킵

- **Task MD102: Vercel Cron 동기화 라우트 구현**
  - `app/api/cron/sync-results/route.ts`: 결과 동기화 전용 엔드포인트
  - 킥오프+2.5h 이후의 SCHEDULED/TIMED 상태 경기만 API 호출하여 갱신
  - 순위표(standings)도 함께 갱신
  - API 할당량 안전장치 (10요청/분 초과 방지)

- **Task MD103: 경기 일정 일일 동기화**
  - `app/api/cron/sync-schedule/route.ts`: 하루 1회 킥오프 시간 갱신
  - 일정 변경/연기 시 킥오프 시간 업데이트 → 동적 Cron 스케줄 자동 반영

---

## Phase HP3: HOME 패널 재구성 — 예정 (우선순위 4)

> **목표**: HOME을 ScoutLab 쇼케이스 중심으로 재구성. 확정된 패널만 구현, 보류 패널은 빈 자리.

- **Task HP301: 기존 패널 정리**
  - LeagueLeadersPanel 제거 (Ranking 페이지와 중복)
  - TopScorersPanel 제거 (PL 한정, 향후 5대 리그로 대체)
  - ScoutlabSpotlight 제거 (정적 소개 → 실제 데이터로 대체 예정)

- **Task HP302: "5대 리그 순위 한눈에" 패널**
  - 5개 리그 각 상위 3~4팀 미니 순위표
  - 우승 경쟁 구도가 바로 보이는 비주얼 (승점 차이 강조)
  - 클릭 → Ranking 페이지로 이동

- **Task HP303: "이번 라운드/오늘 경기" 패널 통합**
  - 기존 TodayMatchesPanel을 "이번 라운드 + 오늘 경기" 통합 패널로 확장
  - 경기 없는 날: 다음 경기 정보 표시
  - 클릭 → Matchday 페이지로 이동

---

## Phase RK2: RANKING UCL/UEL 순위 확장 — 예정 (우선순위 5)

> **목표**: Ranking 페이지에 Champions League, Europa League 순위표 탭 추가.

- **Task RK201: UCL/UEL 순위 API 연동**
  - football-data.org 무료 플랜에서 CL/EL standings 엔드포인트 호출
  - `lib/api/football-data/standings.ts` 확장

- **Task RK202: UCL/UEL 순위표 UI**
  - `ranking-content.tsx`: UCL/UEL 탭 추가
  - 36팀 리그 페이즈 순위표 형태
  - 녹아웃 진출 하이라이트 규칙

- **Task RK203: 동기화 + 검증**
  - UCL/UEL 순위 데이터 동기화 Cron 추가
  - 탭 전환 검증
...
## Phase NW: NEWS 이적뉴스/팀소식 구현 — 예정 (우선순위 7)

> **목표**: NEWS 페이지에 트위터/기사 등 외부 소스의 이적 뉴스, 팀 소식 큐레이션.

- **Task NW101: 뉴스 데이터 소스 + 수집 전략 결정**
  - Twitter API / RSS 피드 / 뉴스 API 조사
  - 자동 수집 vs 수동 큐레이션 결정

- **Task NW102: NEWS 페이지 UI 구현**
  - Coming Soon 제거 → 뉴스 카드 목록
  - 카테고리 필터 (이적/팀소식/부상)
  - 코믹 디자인 적용

- **Task NW103: HOME 트렌딩 뉴스 패널 연동**
...
| F225    | NEWS 이적뉴스/팀소식      | Task NW101~NW103 |

---

**최종 업데이트**: 2026-05-25 (페이지 아키텍처 리디자인 — ScoutLab 중심 전환, 7개 신규 Phase 추가)
**진행 상황**: Phase 1~5A 레거시 완료 ✅ | Phase 6~7E 완료 ✅ | Phase S1~S5 완료 ✅ | Phase N1~N2 완료 ✅ | Phase AF+FD 완료 ✅ | Phase HP+HP2 완료 ✅ | Phase RK+PD 완료 ✅ | **Phase S6 진행 중** → UX1 → UX2 → MD → HP3 → RK2 → RK3 → MP → NW → S7 → S8 → CV
