...
- **Task S603: Bundesliga 전체 스크래핑** ✅
  ...
  - 334명, 1,336건 (per90/padj + per90/raw + total/padj + total/raw × 334명)
  - 1차 실패 22명 (Köln 2명 P90 타임아웃, Wolfsburg 20명 세션 만료) → 팀 단위 재시도 + 개별 재시도로 전원 복구
  - Kilian Fischer: similarity 파싱 WARN (메트릭은 정상)

- **Task S604: Ligue 1 전체 스크래핑** ✅
  - `npm run scrape:scoutlab -- --league="Ligue 1" --match-position`
  - 사전 준비: 기존 AM/W 메트릭 DELETE 후 실행
  - 1차 스크래핑 완료 → 52명 실패 → 재스크래핑 48명 성공
  - 최종 재실패 3명 (ScoutLab 데이터 미존재): Mostafa Mohamed, Breel Embolo, Samir El Mourabet
...
- **Task S606: Similarity 보완 수집** ✅
  - `--match-position`이 메트릭 + Similarity 동시 수집하므로 별도 수집은 누락분만 대상
  - `--similarity-only` 플래그로 누락 선수 팀 단위 보완
  - 누락 34명 (PL 30 + Bundesliga 2 + Ligue 1 2) → 7팀 순회 → 전원 수집 완료
  - 최종: 5대 리그 1,519명 전원 similarity 데이터 보유 (누락 0)

- **Task S607: 데이터 검증 + Scatter/Ranking 통합 테스트** ✅
  - **데이터 이슈 3건 발견 및 수정:**
    - 리그 오기록 34명 (Lille/Paris FC/Rennes/Strasbourg → PL→Ligue 1 UPDATE)
    - Rudy Matondo 중복 (메트릭 없는 Ligue 1 버전 삭제, 메트릭 있는 버전 Ligue 1로 이전)
    - PL 메트릭 없는 선수 13명 (S606 similarity-only에서 생성, 향후 재스크래핑 대상)
    - PL 45명 per90/padj만 (Phase S5 잔여, 기본 모드이므로 Scatter/Ranking 동작 이상 없음)
  - **최종 리그별 현황:**
    | 리그 | 선수 | 메트릭 보유 | Similarity | 포지션 |
    |------|------|-----------|-----------|--------|
    | Bundesliga | 333 | 333 (100%) | 333 | 5종 균등 |
    | La Liga | 391 | 391 (100%) | 391 | 5종 균등 |
    | Ligue 1 | 285 | 281 (99%) | 285 | 5종 균등 |
    | Premier League | 373 | 360 (97%) | 373 | 5종 균등 |
    | Serie A | 136 | 136 (100%) | 136 | 5종 균등 |
  - **E2E 브라우저 테스트 (Playwright):**
    - [✓] Scatter: 전체 리그 산점도 (5대 리그 범례 + 색상 구분)
    - [✓] Scatter: La Liga 필터 (노란색 점만 표시)
    - [✓] Scatter: Ligue 1 필터 (리그 수정 반영 확인)
    - [✓] Ranking: 전체 리그 랭킹 (PL/BL/LL/SA 태그 혼합)
    - [✓] Ranking: Bundesliga 필터 (BL 태그만 표시)
    - [✓] Ranking: Ligue 1 필터 (Rennes/Strasbourg/Lille/Paris FC 선수 정상 표시)
    - [✓] Compare: 기본 Haaland 로드 + 크로스리그 선수 검색
    - [✓] Compare: Haaland(PL) vs Mbappé(La Liga) 전 카테고리 메트릭 비교
...
- **Task S701: Action Maps 스크래퍼 구현** ✅ - 완료
  - ✅ `scripts/scraper/lib/parsers.ts`: Action Maps 탭 파싱 (서버사이드 PNG 이미지 URL 추출)
  - ✅ `scripts/scraper/lib/db.ts`: `upsertActionMaps()` + 이미지 다운로드 → Supabase Storage 업로드
  - ✅ `scripts/scraper/main.ts`: `--action-maps-only` CLI 모드 + `--dump-action-maps-dom` 디버그 모드
  - ✅ DB 마이그레이션: `image_url TEXT` 컬럼 추가 + `scoutlab-action-maps` Storage 버킷
  - ✅ Action Maps UI 개선: 원본 이미지 3등분 crop + 피치 라인 SVG 오버레이 정확 정렬
    - 이미지 픽셀 분석으로 피치 터치라인 좌표 특정 (x=54~458, y=53~678, 섹션=487px)
    - SVG 오버레이를 `preserveAspectRatio="none"` + 퍼센트 기반 absolute 배치로 원본 피치에 정렬
    - `aspectRatio: 68/96`으로 하단 범례 완전 제거
    - 헤더에 총 횟수/P90 표시: "Carries (165 | 6.9 P90)" 형식
    - DB `total_count`/`per90` 자동 계산 (lines 배열 길이 + minutes_played 기반)
  - ℹ️ ScoutLab은 matplotlib 차트를 PNG로 렌더링 → SVG 좌표 추출 불가, 이미지 저장 방식 채택
  - ℹ️ Gemini Vision 좌표 추출(S707)은 정확도 부족 → 원본 이미지 직접 표시 방식 채택

- **Task S702: Action Maps PL 전체 수집** ✅ - 완료
  - ✅ PL 372명 × 3타입 = 1,116행 수집 완료 (이미지 + Storage 업로드)
  - ✅ count/p90 배치 추출: Sharp 전처리 + Tesseract OCR 파이프라인 (`scripts/extract-action-text.ts`)
    - 이미지 상단 45px 크롭 → negate → normalise → 5x 확대 → greyscale → sharpen → OCR
    - 1,031/1,116 레코드 업데이트, 나머지 85개는 실제 값이 0인 정상 데이터
  - ✅ Action Maps 범례 확장: Start→End 그라데이션, Low/High xT 원 크기, Progressive(핑크), xT>0.02(시안)
    - 카드별 범례 → 하단 공유 `ActionMapLegend` 컴포넌트로 통합
...
- **Task S706: Player Card UI 디테일** ✅ - 완료
  - ✅ 선수 검색에 리그/팀 필터 연동 (팀 선택 시 해당 팀 선수 후보 자동 표시)
  - ✅ 검색 API `league`/`team` 쿼리 파라미터 지원 추가
  - ✅ 필터바(시즌/리그/팀) + 선수 검색을 한 줄로 배치 (UX 개선)
  - ✅ layout에서 글로벌 검색 제거 → 각 페이지에 인라인 배치
  - ✅ 전체 하위 탭(Radar, Summary, Similarity, Progression, Action Maps, Compare)에 필터바 통합

- **Task S707: Action Maps 올챙이(Tadpole) 좌표 추출** ✅ - 완료
  - ✅ Claude Vision API(Haiku)로 PNG 이미지에서 개별 올챙이 시작점/끝점/색상/xT 추출
  - ✅ `scripts/scraper/lib/vision-extractor.ts`: Vision API 호출 + 응답 JSON 파싱 + 좌표 검증
  - ✅ `scripts/scraper/lib/anthropic-client.ts`: Anthropic SDK 싱글턴 클라이언트
  - ✅ `scripts/scraper/lib/vision-rate-limiter.ts`: 분당 50회 슬라이딩 윈도우 레이트 리미터
  - ✅ `scripts/scraper/main.ts`: `--extract-lines` CLI 옵션 통합
  - ✅ `scripts/extract-action-lines.ts`: 배치 추출 CLI (`npm run extract:action-lines`)
  - ✅ `types/scoutlab.ts` + `scripts/scraper/lib/types.ts`: `xt` 필드 추가
  - ✅ `action-map-overlay.tsx`: 올챙이 시각화 (테이퍼 라인 + xT 반투명 원)
...
- **Hotfix HP4-01: 섹션 제목 폰트 사이즈 키우기** ✅ - 2026-05-26
  - ✅ `comic-panel.tsx` `ComicPanelTitle`: 제목 `--comic-text-lg`(18px) → `--comic-text-xl`(22px)
  - ✅ `comic-panel.tsx` `ComicPanelTitle`: 부제목 `--comic-body-base`(13px) → `--comic-body-lg`(15px)
  - ✅ 본문(13~16px) 대비 제목(22px) 시각적 계층 구분 강화
...
> **목표**: 프로덕션 환경에서 발견된 버그 수정

### Task BF01: 인증 리다이렉트 localhost 문제 수정 ✅

- ✅ `getSiteUrl()` 유틸리티 함수 추가 (`NEXT_PUBLIC_SITE_URL` > `NEXT_PUBLIC_VERCEL_URL` > `window.location.origin`)
- ✅ `oauth-buttons.tsx` — `window.location.origin` → `getSiteUrl()`
- ✅ `sign-up-form.tsx` — 동일 수정
- ✅ `forgot-password-form.tsx` — 동일 수정
- ✅ Supabase 대시보드 Site URL → `https://pitch-ac.vercel.app` 변경
- ✅ Vercel 환경변수 `NEXT_PUBLIC_SITE_URL` 추가

### Task BF02: 홈 페이지 시즌 종료 시 다음 대회 일정 표시 ✅

- ✅ `getUpcomingFixtures()` 함수 추가 — 전체 대회(5대 리그+UCL)에서 NS 상태 경기 조회
- ✅ `home-content.tsx` — PL 라운드 경기 모두 FT일 때 `getUpcomingFixtures()` fallback
- ✅ `round-matches-panel.tsx` — "COMING UP" + 대회명(UCL 등) + 날짜 표시
- ✅ `round-matches-panel.tsx` — 오프시즌 fallback "WORLD CUP 2026 is coming soon!" 메시지

### Task BF03: ScoutLab 기본 선수(Haaland) 로드 ✅

- ✅ `scoutlab-repository.ts` — `getDefaultScoutlabPlayer()` 함수 추가 (이름으로 Haaland 조회, React `cache` 적용)
- ✅ `scoutlab-constants.ts` — `DEFAULT_PLAYER_NAME` 상수 추가
- ✅ 7개 페이지 수정: Player Card, Summary, Progression, Action Maps, Compare, Radar, Similarity
  - `playerId` 미지정 시 Haaland 데이터 기본 표시
  - Haaland가 DB에 없을 때만 기존 EmptyState 유지
- ✅ Radar, Summary 페이지 EmptyState 조건 수정 (`!params.playerId` → `!selectedPlayer`)

### Task BF04: 페이지 전환 로딩 인디케이터 추가 ✅

- ✅ `nextjs-toploader` 패키지 설치
- ✅ `app/layout.tsx` — `<NextTopLoader>` 추가 (comic-red 색상, 스피너 비활성화)
- ✅ 페이지/탭 전환 시 상단 프로그레스 바 자동 표시

### Task BF05: sync-news 워크플로우 타임아웃 수정 + 프로젝트 정리 ✅

- ✅ `sync-news.yml` — `timeout-minutes` 10→20 (npm 캐시 1.4GB 다운로드에 ~9분 소요, 이전 3회 연속 타임아웃 cancelled)
- ✅ `.gitignore` — 루트 임시 파일 패턴 추가 (`/*.png`, `/*.jpg`, `/*.log`, `/*.yml`, `/test-results/`)
- ✅ 루트 디렉토리 임시 파일 219개 삭제 (테스트 스크린샷, 스크래핑 로그)

### Task BF06: 시즌 종료 후 종료된 라운드가 "THIS ROUND"로 표시되는 버그 수정 ✅

- ✅ `home-content.tsx` — 라운드 경기 모두 FT일 때 `nextRoundFixtures`를 무조건 비우도록 수정
  - 기존: `upcomingFixtures`가 있을 때만 `nextRoundFixtures = []` → 시즌 완전 종료 시 종료된 GW가 "Coming up"으로 표시
  - 수정: `hasUpcomingInRound === false`이면 즉시 `nextRoundFixtures = []` → OFF-SEASON 패널 정상 표시

### Task BF07: 스켈레톤 로딩 인디케이터 개선 ✅

- ✅ `components/page-loading-indicator.tsx` — 신규 컴포넌트 (Loader2 스피너 + "Loading..." 텍스트)
- ✅ 기존 `loading.tsx` 3개에 인디케이터 추가 (Scouting, Matchday, Matchday/[fixtureId])
- ✅ 인라인 Suspense fallback 3개에 인디케이터 추가 (HomeContentSkeleton, RankingFallback, NewsSkeleton)
- ✅ 누락된 `loading.tsx` 2개 신규 생성 (Ranking, News)
- `nextjs-toploader` 진행 바 완료 후에도 스피너가 계속 표시되어 사용자가 데이터 로딩 중임을 인지 가능
...
| **크롤링 실행**    | GitHub Actions self-hosted runner 하루 3회 자동 (KST 09/15/21시) + `npm run sync:news` 수동 |
...
| **HOME 연동**      | ✅ Task NW105에서 완료 — 최신 3건을 홈 뉴스 패널에 Server Component로 표시                  |
| **자동 동기화**    | ✅ Task NW106에서 완료 — GitHub Actions self-hosted runner (cron 하루 3회)                  |
...
### Task NW105: HOME 트렌딩 뉴스 패널 연동 ✅

- ✅ `lib/repositories/news-repository.ts` — `getLatestNews(limit)` Server Component용 Supabase 직접 쿼리
- ✅ `home-content.tsx`에서 `Promise.all`로 뉴스 3건 병렬 fetch
- ✅ `home-news-panel.tsx` — 컴팩트 뉴스 카드 3개 + "MORE NEWS →" 버튼
  - ✅ 소스 타입 뱃지 + 상대 시간 + 조회/추천/댓글 메타
  - ✅ 첫 번째 소스 링크 도메인 표시
  - ✅ 뉴스 없을 때 빈 상태 UI
- ✅ `NewsPlaceholder` (COMING SOON) → `HomeNewsPanel` 교체

### Task NW106: GitHub Actions 자동 동기화 ✅

- ✅ `.github/workflows/sync-news.yml` — 하루 3회 cron (KST 09:00/15:00/21:00) + 수동 실행
- ✅ Self-hosted runner 사용 (FMKorea Cloudflare Turnstile이 클라우드 IP 차단 → 로컬 Mac 러너로 우회)
- ✅ `~/actions-runner` launchd 서비스 등록 (Mac 부팅 시 자동 시작)
- ✅ 크롤러 봇 감지 우회 강화
  - `--disable-blink-features=AutomationControlled` Chromium 플래그
  - `navigator.webdriver` 속성 false 오버라이드
  - User-Agent / viewport / locale 설정
- ✅ 실패 시 `debug-screenshot.png` 아티팩트 업로드 (디버깅용, retention 3일)
...
| 레이어              | 전략                                   | TTL                                       |
| ------------------- | -------------------------------------- | ----------------------------------------- |
| DB (Supabase)       | 크롤링 스크립트가 주기적 저장          | GitHub Actions 하루 3회 자동              |
...
| F225    | NEWS 이적뉴스 큐레이션 ✅ | Task NW100~NW106 |

---

**최종 업데이트**: 2026-06-03 (Phase S7 — Task S706 전체 하위 탭 필터바 통합 완료)
**진행 상황**: Phase 1~5A 레거시 완료 ✅ | Phase 6~7E 완료 ✅ | Phase S1~S5 완료 ✅ | Phase N1~N2 완료 ✅ | Phase AF+FD 완료 ✅ | Phase HP+HP2+HP3+HP4 완료 ✅ | Phase RK+RK2+PD 완료 ✅ | Phase UX1~UX2 완료 ✅ | Phase MD 완료 ✅ | Phase SB 완료 ✅ | Phase S6 완료 ✅ | Phase NW 완료 ✅ | Phase BF 완료 ✅ | **Phase S7 진행 중 (S701, S702, S706, S707 완료)** → S703 → RK3 → MP → S8 → CV
