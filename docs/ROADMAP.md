...
- **Task 002: TypeScript 타입 정의 및 더미 데이터 구축**
  ...
  - [ ] `lib/mock/` 디렉토리에 더미 데이터 생성:
    - 4~6개 팀 더미 데이터
    - 10~15명 선수 더미 데이터 (포지션별 분포)
    - 1개 게임위크 경기 더미 데이터 (6경기, 예정/라이브/종료 혼합)
    - 맥락 데이터 포함 (순위, 백분위, 전년 비교)
  - [ ] 더미 데이터에서 실제 PL 선수·팀 이름 사용 (UI 현실감 확보)
  - **Playwright 검증**:
    - [ ] 개발 서버 실행 후 `/matchday`, `/players`, `/compare` 접속 시 콘솔 에러 없음
    - [ ] TypeScript 컴파일 에러로 인한 빌드 실패 없음 (빌드 오류 시 빨간 에러 오버레이 미표시)
    - [ ] 더미 데이터가 적용된 페이지에서 실제 PL 팀명 (예: "Arsenal", "Chelsea") 텍스트 확인
  - **관련 기능**: 전체
  - **의존성**: 없음
...
- **Task 003: 글로벌 내비게이션 구현 (F013)**
  ...
  - **Playwright 검증**:
    ...
    - [ ] 헤더 "Matchday" 링크 클릭 → `/matchday` 이동, 링크 active 스타일 확인
    - [ ] 헤더 "Players" 링크 클릭 → `/players` 이동, 링크 active 스타일 확인
    - [ ] 헤더 "Compare" 링크 클릭 → `/compare` 이동, 링크 active 스타일 확인
    - [ ] 테마 토글 클릭 → `<html>` 태그 class가 `dark` ↔ `light` 전환 확인
    - [ ] 미인증 상태: "Sign in", "Sign up" 버튼 표시 확인
    - [ ] 모바일 하단 탭 "More" 클릭 → `/more` 이동
  - **관련 기능**: F013
  - **의존성**: Task 001
...
- **Task 004: 매치데이 대시보드 UI (F001)**
  ...
  - **Playwright 검증**:
    ...
    - [ ] 페이지 초기 로드 시 스켈레톤 UI 표시 후 실제 카드로 교체 확인
  - **관련 기능**: F001
...
- **Task 006: 선수 검색 페이지 UI (F011)**
  - [ ] 검색 입력 필드 (shadcn Command 컴포넌트 기반)
...
- **Task 007: 선수 프로필 페이지 UI (F006, F008)**
  ...
  - **Playwright 검증**:
    ...
    - [ ] 각 스탯 카드에 리그 순위 텍스트(예: "3rd") 표시 확인
    - [ ] 각 스탯 카드에 백분위 프로그레스 바 요소 존재 확인
    - [ ] 전년 비교 방향 아이콘(↑ 또는 ↓) 텍스트 표시 확인
    - [ ] 스탯 수치 옆 [?] 아이콘 버튼 존재 확인
    - [ ] 최근 폼 스파크라인 SVG 또는 차트 요소 렌더링 확인
    - [ ] "Compare" 버튼 클릭 → `/compare?p1=[playerId]` URL로 이동 확인
    - [ ] 페이지 초기 로드 시 스켈레톤 UI 표시 후 실제 데이터로 교체 확인
  - **관련 기능**: F006, F008
  - **의존성**: Task 002, Task 003
...
**📅 최종 업데이트**: 2026-03-21
**📊 진행 상황**: Phase 1 진행 중 (1/23 Tasks 완료)
