...
- **Task 019: 맥락 데이터 계산 엔진 구현** ✅ — 완료
  - ✅ 리그 내 순위 계산 (포지션별 — GK/DEF/MID/FWD 독립 순위)
  - ✅ 백분위 계산 (포지션 내 상대적 위치, 1위→100, 꼴찌→0)
  - ✅ 전년 시즌 비교 데이터 생성 (2024/2025 시즌 데이터 기반 prevSeason 저장)
  - ✅ 계산 결과를 `player_season_stats.context` JSONB에 저장하는 배치 로직
  - ✅ 데이터 동기화 Cron과 연계 (sync-stats 07:00 UTC → calculate-context 07:30 UTC)
  - ✅ 레이더 차트 `positionAverage` 50 고정 → 실제 포지션 평균으로 갱신
  - **구현 세부사항**:
    - `lib/services/sync/calculate-context.ts` — 계산 엔진 핵심 (신규)
    - `app/api/cron/calculate-context/route.ts` — Cron 엔드포인트 (신규)
    - `lib/services/sync/index.ts` — `calculateContext` export 추가
    - `vercel.json` — `calculate-context` Cron 스케줄 추가 (30 7 \* \* \*)
    - Standard Competition Ranking 동점 처리 (ex: 두 선수 6골 동점 → 공동 2위, 다음 순위 4위)
    - 전년(2024/2025) 데이터 없음 → `prevSeason: null` → UI "첫 시즌" 배지
  - **Playwright MCP 직접 브라우저 검증** ✅:
    - ✅ `/players/16827155` (Bukayo Saka) → 골 "리그 1위", 백분위 100% 표시 확인
    - ✅ `/players/32612` (Kai Havertz) → 골 "리그 2위" 94%, 드리블 "리그 7위" 63% 등 다양한 순위 확인
    - ✅ 백분위 프로그레스 바 값 0~100 범위 내 확인
    - ✅ 전년 데이터 없음 → "첫 시즌" 배지 표시 확인 (신규 선수 처리 정상)
    - ✅ xG/xA → N/A (context 미계산, Starter 플랜 미지원 — 정상)
    - ✅ 헤더 카드 평점에 "리그 N위 · 상위 N%" 텍스트 표시 확인
    - ✅ DB 검증: 포지션별 순위 정확성 및 동점 처리 확인 (Eze/KDH MID 골 공동 2위)
  - **관련 기능**: F006 (맥락 스탯의 핵심)
  - **의존성**: Task 011, Task 015

- **Phase 3 코드 리뷰 후속 수정** ✅ — 완료 (2026-03-21)
  - ✅ compare 페이지 mock 데이터 → Supabase 실데이터 연동 (`compare/page.tsx`, `compare-client.tsx`)
  - ✅ `sync-stats` 전체 선수 배치 처리로 offset 버그 수정 (기존 BATCH_SIZE 50 제한으로 일부 선수 누락 문제)
  - ✅ `sync-players` BATCH_SIZE 제한 제거 및 에러 로깅 보완
  - ✅ `calculate-context` 순차 UPDATE → 배치 upsert 최적화 (성능 개선)
  - ✅ `getCurrentGameweek` 3개 순차 쿼리 → `Promise.all` 병렬화 (성능 개선)

---

### Phase 4: 고급 기능 및 최적화

- **Task 020: 라이브 스코어 실시간 시스템 고도화 (F002)** ✅ — 완료 (2026-03-21)
  - ✅ 라이브 경기 감지 및 폴링 주기 자동 전환 (60초 ↔ 5분)
  - ✅ SportMonks Livescores 엔드포인트 최적화 (라이브 오버레이 패턴 + 서버 인메모리 캐시 30초)
  - ✅ 라이브 이벤트 발생 시 시각적 피드백 (sonner 골 알림 toast, ScoreFlash 애니메이션)
  - ✅ 경기 종료 감지 후 폴링 중단 및 최종 데이터 갱신 (FT writeback + 탭 자동 전환)
  - **Playwright 검증**:
    - ✅ `data-fixture-id` 10개 카드 확인, 비라이브 시 `data-live` 미설정 확인
    - ✅ API `hasLive: false` → 5분 폴링 / Mock `hasLive: true` → 60초 폴링 로직 확인
    - ✅ Sonner Toaster 컨테이너(`region[aria-label="Notifications alt+T"]`) DOM 마운트 확인
    - ✅ FT 경기 → "FT" 배지 표시, "포스트매치" 탭 자동 선택, AutoRefreshIndicator 미표시
  - **관련 기능**: F002
  - **의존성**: Task 016

- **Task 021: 비교 카드 OG 이미지 생성 및 공유 (F010)** ✅ — 완료 (2026-03-21)
  - ✅ `next/og` 기반 OG 이미지 동적 생성 API 라우트 (`/api/og?p1=X&p2=Y`)
  - ✅ 배틀카드 디자인 템플릿 (양 선수 사진, 핵심 스탯 비교, 승자 컬러 표시)
  - ✅ "Compare on pitch-ac" 워터마크 적용
  - ✅ 이미지 다운로드 기능 (fetch → blob → `<a download>` 패턴)
  - ✅ 비교 URL 공유 시 OG 메타태그 자동 설정 (`generateMetadata`)
  - **Playwright 검증**:
    ...
    - ✅ "Share as Image" 버튼 클릭 → 이미지 다운로드 트리거 확인 (blob URL + 파일명 자동 설정)
  - **관련 기능**: F010
  - **의존성**: Task 009, Task 018

- **Task 022: 성능 최적화** ✅ — 완료 (2026-03-21)
  - ✅ 이미지 최적화: `unoptimized` 제거 → `/_next/image` WebP 변환 활성화 (4개 파일)
  - ✅ 번들 사이즈 최적화: recharts `next/dynamic` 지연 로딩 (Client Component 래퍼 패턴)
  - ✅ API 응답 캐싱: `Cache-Control` 헤더 추가 (라이브 `s-maxage=10`, 비라이브 `s-maxage=30~60`)
  - ✅ TanStack Query `staleTime: 30_000` 추가 (불필요한 refetch 방지)
  - ✅ 로딩 상태 개선: `compare/loading.tsx` 신규 + Suspense fallback 스켈레톤
  - **Playwright MCP 직접 브라우저 검증** ✅:
    - ✅ `/players` 선수 사진 → `/_next/image?url=...` 경유 확인 (WebP 최적화 활성화)
    - ✅ `/players/[id]` → recharts 레이더 차트 dynamic import 후 정상 렌더링
    - ✅ `/compare?p1=...&p2=...` → 비교 배틀카드 + recharts 레이더 차트 정상 렌더링
    - ✅ `/api/matchday/fixtures?gw=31` → `Cache-Control: public, s-maxage=30, stale-while-revalidate=60`
    - ✅ `/api/matchday/fixture?id=...` (FT) → `Cache-Control: public, s-maxage=60, stale-while-revalidate=120`
    - ✅ 전체 페이지 콘솔 에러 0개
  - **관련 기능**: 전체
  - **의존성**: Phase 3 완료

- **Task 023: 프로덕션 배포 및 모니터링** ✅ — 완료 (2026-03-21)
  - ✅ 글로벌 에러 바운더리: `app/error.tsx`, `app/global-error.tsx` (WSOD 방지)
  - ✅ 글로벌 404 페이지: `app/not-found.tsx`
  - ✅ 보안 HTTP 헤더: X-Frame-Options, X-Content-Type-Options, Referrer-Policy 등 (`next.config.ts`)
  - ✅ 미사용 이미지 도메인 제거: `media.api-sports.io` 제거
  - ✅ SEO: `app/robots.ts` (크롤러 접근 제어), `app/sitemap.ts` (동적 사이트맵)
  - ✅ 루트 메타데이터 보강: `title.template`, openGraph, twitter 카드 (`app/layout.tsx`)
  - ✅ 페이지별 generateMetadata: matchday(GW 동적), players(정적), players/[id](선수 동적), matchday/[id](경기 동적)
  - ✅ `.env.example` 환경변수 문서화
  - ✅ ESLint Next.js 규약 파일 예외 보완: `global-error.tsx`, `robots.ts`, `sitemap.ts`
  - **Playwright 검증** ✅:
    - ✅ 페이지 소스 → `<title>`, `<meta name="description">`, OG 태그 존재 확인
    - ✅ 응답 헤더 → `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff` 확인
    - ✅ `/robots.txt` → 크롤러 규칙 출력 확인
    - ✅ `/sitemap.xml` → 선수 프로필 URL 포함 확인
    - ✅ `npm run validate` 통과 (type-check + lint + format)
  - **관련 기능**: 전체
  - **의존성**: Phase 3 완료
...
### Phase 5-A: 안정성 및 코드 품질 ✅

> **배경**: Phase 4 완료 후 전체 코드 리뷰 + Playwright MCP 프로덕션 테스트(10개 시나리오 전수 통과)에서 도출된 개선 사항.

- **Task 024: 비교 페이지 title 중복 버그 수정** ✅ — 완료 (2026-03-21)
  - ✅ `compare/page.tsx` generateMetadata에서 `| pitch-ac` 중복 제거 (layout template과 충돌)
  - ✅ `getPlayerSeasonStats` 개별 호출 → `seasonStatsMap` 재활용 (DB 쿼리 2회 절약)
  - ✅ `npm run validate` 통과 확인
  - **발견 경위**: Playwright MCP 테스트 #7에서 `"Bukayo Saka vs Kai Havertz | pitch-ac | pitch-ac"` 확인

- **Task 025: React cache()를 활용한 요청 중복 제거** ✅ — 완료 (2026-03-22)
  - ✅ `lib/repositories/fixture-repository.ts` — `getCurrentGameweek()`, `getFixtureById()` cache 래핑
  - ✅ `lib/repositories/player-repository.ts` — `getPlayerById()`, `getPlayerSeasonStats()` cache 래핑
  - ✅ `lib/repositories/standing-repository.ts` — `getStandingsByTeamIds()` cache 래핑
  - ✅ `lib/repositories/team-repository.ts` — `getTeamsByIds()` cache 래핑
  - ✅ `import { cache } from "react"` 기반 요청 범위 내 캐싱 적용 — generateMetadata + 페이지 컴포넌트 이중 호출 제거
  - **의존성**: 없음

- **Task 026: 경기 상세 데이터 로딩 중복 제거** ✅ — 완료 (2026-03-22)
  - ✅ `lib/services/fixture-detail-service.ts` 신규 — `assembleFixtureDetail()` 공통 함수 추출
  - ✅ `app/(app)/matchday/[fixtureId]/page.tsx` — 중복 로직 제거, `assembleFixtureDetail()` 호출
  - ✅ `app/api/matchday/fixture/route.ts` — 중복 로직 제거, `assembleFixtureDetail()` 호출
  - ✅ `types/fixture.ts` — `FixtureDetailData` 타입을 공용 타입으로 이동
  - **의존성**: 없음

- **Task 027: 레포지토리 레이어 server-only 강화** ✅ — 완료 (2026-03-22)
  - ✅ `lib/repositories/*.ts` 전체 6개 파일에 `import "server-only"` 추가 (index, fixture, player, team, standing, injury, mappers)
  - ✅ `lib/services/sync/*.ts` 전체 8개 파일에 `import "server-only"` 추가 (auth, log, db-mappers, retry, sync-teams/fixtures/players/stats)
  - ✅ `lib/services/h2h.ts`, `lib/services/fixture-detail-service.ts`에도 적용
  - ✅ 클라이언트 번들 포함 방지 빌드 타임 검증
  - **의존성**: 없음

- **Task 028: CSP(Content-Security-Policy) 헤더 추가** ✅ — 완료 (2026-03-22)
  - ✅ `next.config.ts`에 CSP `Content-Security-Policy-Report-Only` 헤더 설정
  - ✅ `img-src`: `cdn.sportmonks.com`, `blob:`, `data:` 허용
  - ✅ `connect-src`: `*.supabase.co`, `wss://*.supabase.co` 허용
  - ✅ `frame-ancestors: 'none'`, `object-src: 'none'`, `base-uri: 'self'` 보안 강화
  - ✅ 추후 strict CSP(nonce 기반)로 전환 예정
  - **의존성**: 없음

- **Task 030: 단위 테스트 (Vitest)** ✅ — 완료 (2026-03-22)
  - ✅ `lib/repositories/__tests__/mappers.test.ts` — DB row → 앱 타입 변환 함수 테스트 (255줄)
  - ✅ `lib/services/sync/__tests__/calculate-context.test.ts` — 순위/백분위 계산 로직 테스트 (74줄)
  - ✅ `lib/api/sportmonks/__tests__/mappers.test.ts` — SportMonks API 매퍼 테스트 (191줄)
  - ✅ `app/(app)/matchday/__tests__/_utils.test.ts` — 날짜 유틸 테스트 (93줄)
  - ✅ `lib/__tests__/server-only-mock.ts` — server-only 모듈 테스트용 mock
  - **의존성**: 없음

- **Task 031: 에러 모니터링 연동 (Sentry)** ✅ — 완료 (2026-03-22)
  - ✅ `@sentry/nextjs` v10.45 설치 및 `next.config.ts` `withSentryConfig` 래핑
  - ✅ `sentry.server.config.ts`, `sentry.edge.config.ts` — 서버/엣지 런타임 설정
  - ✅ `instrumentation.ts`, `instrumentation-client.ts` — Next.js instrumentation 훅 설정
  - ✅ `app/error.tsx` — `console.error` → `Sentry.captureException` 교체
  - ✅ `app/global-error.tsx` — `console.error` → `Sentry.captureException` 교체
  - ✅ DSN 없으면 Sentry 비활성화 (로컬 개발 시 사일런트), 소스맵 CI 전용 업로드
  - **의존성**: 없음
...
- **Task 032: 즐겨찾기 팀/선수 (로그인 사용자)**
  - Supabase `user_favorites` 테이블 추가
  - 매치데이 대시보드에서 즐겨찾기 팀 경기 상단 고정
  - 선수 프로필에 즐겨찾기 버튼
  - **의존성**: Task 027

- **Task 033: 리그 순위표 전체 뷰 페이지**
  - `/standings` 라우트 추가
  - 20팀 전체 순위표 (승점, 승/무/패, 득실차, 최근 5경기 폼)
  - 모바일 반응형 가로 스크롤
  - **의존성**: 없음

- **Task 034: 서버 사이드 선수 검색 + 무한 스크롤**
  - 현재 클라이언트 사이드 전체 선수 로딩 → 서버 검색 API 전환
  - `GET /api/players/search?q=...&cursor=...` 엔드포인트
  - TanStack Query `useInfiniteQuery` 기반 무한 스크롤
  - 초기 페이로드 대폭 감소 (~550명 → 20명/페이지)
  - **의존성**: 없음

- **Task 035: 선수 비교 히스토리 (최근 비교 목록)**
  - localStorage 기반 최근 비교 5개 저장
  - 비교 페이지 하단에 히스토리 카드 표시
  - **의존성**: 없음
...
- **Task 036: API 플랜 업그레이드 (xG/xA 활성화)**
  - SportMonks Starter → Pro 플랜 (4/4 만료 대비)
  - xG/xA 데이터 자동 수집 시작
  - N/A → 실데이터 전환 (타입은 이미 nullable 처리됨)
  - **의존성**: 없음 (비즈니스 결정)

- **Task 037: 폼 분석 대시보드 (최근 N경기 추이 차트)**
  - 선수 프로필 페이지에 "폼 추이" 섹션 추가
  - 최근 10경기 평점/골/어시스트 라인 차트
  - Recharts `LineChart` 활용
  - **의존성**: Task 018

- **Task 038: 포지션별 선수 비교 (같은 포지션 내 순위)**
  - 선수 프로필에서 "같은 포지션 상위 5명" 테이블
  - 포지션 필터 기반 비교 진입점
  - **의존성**: Task 019
...
- **Task 039: PWA (오프라인 캐시, 푸시 알림)**
  - `next-pwa` 또는 커스텀 Service Worker
  - 오프라인 시 마지막 데이터 표시
  - 라이브 경기 골 알림 Push Notification
  - **의존성**: Task 020

- **Task 040: i18n (영어 지원)**
  - `next-intl` 기반 국제화
  - 한국어/영어 2개 언어 지원
  - URL 구조: `/ko/matchday`, `/en/matchday`
  - **의존성**: 없음

- **Task 041: 소셜 기능 (댓글, 예측 투표)**
  - 경기별 댓글 (Supabase Realtime)
  - 경기 결과 예측 투표
  - 사용자 예측 적중률 통계
  - **의존성**: Task 032

- **Task 042: 관리자 대시보드 (sync 모니터링)**
  - `/admin` 라우트 (관리자 전용)
  - sync_logs 시각화 (성공/실패 비율, 최근 동기화 시간)
  - 수동 sync 트리거 버튼
  - **의존성**: Task 015
...
**📅 최종 업데이트**: 2026-03-22
**📊 진행 상황**: Phase 1 완료 ✅ | Phase 2 완료 ✅ | Phase 3 완료 ✅ | Phase 4 완료 ✅ | Phase 5-A 완료 ✅

- **완료**: Task 001~012 (12개), Task 015~028 (14개), Task 030~031 (2개), Phase 3 코드 리뷰 후속 수정, Task 013·014 생략 결정
- **Phase 5-A (안정성/품질)**: 전체 완료 ✅ (Task 024~028, 030~031)
- **Phase 5-B (UX 향상)**: Task 032~035 예정
- **Phase 5-C (분석 확장)**: Task 036~038 예정
- **Phase 5-D (플랫폼 확장)**: Task 039~042 예정
...
> **프로덕션 배포 후 전체 리뷰 (2026-03-21)**: Phase 4 완료 후 전체 코드 리뷰 + Playwright MCP 10개 시나리오 프로덕션 테스트 수행. Critical 이슈(title 중복 버그) 즉시 수정 완료. 15개 개선 사항 도출 → Phase 5 로드맵 수립.
