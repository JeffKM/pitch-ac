...
- ✅ `rate-limiter.ts` — 요청 간 5~8초 랜덤 간격 (봇 감지 회피)
- ✅ `client.ts` — `fmkoreaFetch(path)`: 브라우저 헤더셋(UA/Accept/Referer 등), timeout, rate limiter 연동
...
- ✅ `parsers.ts` — cheerio HTML 파싱 (테이블 기반 `<tr>` 레이아웃)
  ...
  - ✅ `parseArticleUrls(html)`: 본문에서 모든 외부 링크 추출
...
- ✅ 실행 흐름:
  1. ✅ 축구 소식통(`football_news`) 리스트 페이지 크롤링 (1회 요청)
  2. ✅ 각 글 제목 → 금칙어 필터링
  3. ✅ DB 조회 → 이미 있는 글은 메타데이터만 upsert
  4. ✅ 새 글만 → 본문 크롤링 (5~8초 간격) → 링크 추출 → source_type 태깅
...
**최종 업데이트**: 2026-05-26 (Phase NW — NEWS 이적뉴스 큐레이션 완료)
