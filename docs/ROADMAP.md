...
- **Task 007: 선수 프로필 페이지 UI (F006, F008)** ✅
  - [x] 헤더 카드: 선수 사진·이름·클럽·등번호·포지션·국적
  - [x] 맥락 스탯 카드 그리드:
    - 각 수치(골/어시스트/xG/xA) + 리그 순위 + 백분위 바 + 전년 비교(↑↓)
    - 수치 옆 [?] 용어 설명 팝오버 트리거
  - [x] 최근 10경기 폼 스파크라인 + 상승/하락 트렌드 텍스트
  - [x] "Compare" 버튼 → 선수 비교 페이지 이동 (p1 파라미터)
  - [x] 로딩 스켈레톤 UI
  - **Playwright 검증**:
    - [x] `/players/[더미playerId]` 접속 → 선수 이름·클럽·포지션 헤더 렌더링 확인
    - [x] 스탯 카드 그리드 최소 4개(골/어시스트/xG/xA) 렌더링 확인
    - [x] 각 스탯 카드에 리그 순위 텍스트(예: "1위") 표시 확인
    - [x] 각 스탯 카드에 백분위 프로그레스 바 요소 존재 확인
    - [x] 전년 비교 방향 아이콘(↑ 또는 ↓) 텍스트 표시 확인
    - [x] 스탯 수치 옆 [?] 아이콘 버튼 존재 확인
    - [x] 최근 폼 스파크라인 SVG 또는 차트 요소 렌더링 확인
    - [x] "선수 비교" 버튼 → `/compare?p1=[playerId]` URL로 이동 확인
    - [x] 로딩 스켈레톤 UI (loading.tsx) 구현 완료
  - **관련 기능**: F006, F008
  - **의존성**: Task 002, Task 003
